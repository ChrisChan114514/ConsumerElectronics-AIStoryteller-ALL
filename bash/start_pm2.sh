...
readonly APP_ENTRY="${APP_DIR}/src/main.js"
...
show_failure_diagnostics() {
  ...
  if command_exists ss; then
    log "TCP listeners related to port ${SERVICE_PORT}:"
    ss -ltnp 2>/dev/null | grep -E "(^|:)${SERVICE_PORT}([[:space:]]|$)" || true
  fi
}
...
[[ -f "${APP_ENTRY}" ]] || fail "Application entry does not exist: ${APP_ENTRY}"
...
if pm2 describe "${APP_NAME}" >/dev/null 2>&1; then
  log "Removing the previous PM2 application definition..."
  pm2 delete "${APP_NAME}"
fi

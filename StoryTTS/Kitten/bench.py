"""Concurrent HTTP benchmark for the Kitten adapter."""
...
async def run(args: argparse.Namespace) -> None:
    ...
    async with httpx.AsyncClient(timeout=args.timeout) as client:
        async def request() -> None:
            ...
            async with semaphore:
                ...
                try:
                    result = await client.post(f"{args.base_url.rstrip('/')}/audio/speech", json={"input": text, "voice": args.voice, "response_format": "wav"})
    ...
    print(f"requests={args.requests} ok={len(timings)} errors={errors} concurrency={args.concurrency} (server-side MAX_CONCURRENCY controls GPU slots)")
